<template>
    <div class="store-parent my-8">
        ...
        <section v-if="mode === 'selfhost'" class="items-center">
            <h1 class="font-bold text-2xl">New Video Upload Wizard</h1>
            <h2>Upload videos to darel's Video Project server</h2>
            <UForm class="store-content my-4 lg:w-[65vw] center"  :state="state" @submit="goUpload()">
                <input @input="goAction()" type="file" id="fileInput" class="py-4" required accept="video/*"/>
                <video v-if="inputFile" id="upload-preview" autoplay controls muted/>
                <UFormGroup name="title" class="store-item my-4" label="Content title">
                    <UInput type="text" placeholder="Ulang tahun Adib ke-60" v-model="state.title" required/>
                </UFormGroup>
                <UFormGroup name="desc" class="store-item my-4" label="Content description">
                    <UInput type="text" placeholder="Recorded at 32th April, 2062" v-model="state.desc" required/>
                </UFormGroup>
                <UFormGroup name="date_created" class="store-item my-4" label="Date created" description="When is it created?">
                    <UInput type="date" v-model="state.date_created" required/>
                </UFormGroup>
                <UFormGroup name="category_select" class="store-item my-4" label="Category" description="What is the category?">
                    <USelect v-model="state.category_select" :options="categories" option-attribute="name"/>
                </UFormGroup>
                <UFormGroup name="submitted_by" class="store-item my-4" label="Content made by" description="Who created it?">
                    <UInput type="text" placeholder="Who makes the content?" v-model="state.submitted_by" required/>
                </UFormGroup>
                <UButton class="my-2 p-3" variant="outline" type="submit" :disabled="clicked" :loading="uploading">
                    <p>Upload</p>
                </UButton>
            </UForm>
        </section>
        <section v-if="mode === 'youtube'">
            ...
            <UForm class="store-content my-4" :state="state" @submit="goStore()">
                <UFormGroup name="ytLink" class="store-item my-4" label="YouTube Video ID" description="The link of the YouTube video.">
                ...
                <UButton class="my-2 p-3" variant="outline" type="submit">
                    <p>Upload</p>
                </UButton>
            </UForm>
        </section>
...
function goAction() {
    ...
    const videoPlayback = document.getElementById('upload-preview');

    videoPlayback.src = URL.createObjectURL(file);
    videoPlayback.play();
}
watch(() => isHome.value, (newVal) => {
    if(newVal) {
        methods.value = [
            { value: 'selfhost', label: 'Self Host', icon:'i-heroicons:server-stack' },
            { value: 'youtube', label: 'YouTube', icon:'i-heroicons:play-solid' },
